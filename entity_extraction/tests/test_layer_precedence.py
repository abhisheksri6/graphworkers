"""KG-AC-12: layer-precedence span merge — gazetteer > spaCy > LLM on overlap within a chunk;
non-overlapping candidates from every layer unioned; deterministic for fixed input."""
import pytest
...
def test_highest_precedence_wins_on_overlap():
    cands = [
        _c("Acme", "spacy", "c1", 0, 4),          # overlaps the gazetteer span
        _c("Acme Corp", "gazetteer", "c1", 0, 9),  # highest precedence -> kept whole
        _c("Acme Corporation", "llm", "c1", 0, 16),  # overlaps -> dropped
    ]
    kept = merge_candidates(cands)
    assert len(kept) == 1
    assert kept[0].layer == "gazetteer"
...
def test_transitive_overlap_keeps_disjoint_survivor():
    # A(gaz)[0,9] overlaps B(spacy)[5,14]; C(llm)[15,20] overlaps neither A nor B.
    cands = [
        _c("A Corp", "gazetteer", "c1", 0, 9),
        _c("Corp B", "spacy", "c1", 5, 14),
        _c("D Inc", "llm", "c1", 15, 20),
    ]
    kept = merge_candidates(cands)
    surfaces = {k.surface_form for k in kept}
    assert surfaces == {"A Corp", "D Inc"}  # B dropped (overlaps the gazetteer winner)
...
@pytest.mark.ac("KG-AC-12")
def test_regex_layer_precedence_between_gazetteer_and_spacy():
    # evolve v8: gazetteer > regex > spaCy > LLM (clarify 2026-08-06).
    cands = [
...
@pytest.mark.ac("KG-AC-12")
def test_gazetteer_still_beats_regex_on_overlap():
    cands = [
        _c("Acme Corp", "regex", "c1", 0, 9),
        _c("Acme Corporation", "gazetteer", "c1", 0, 16),  # highest precedence -> kept whole
    ]
    kept = merge_candidates(cands)
    assert len(kept) == 1
    assert kept[0].layer == "gazetteer"
